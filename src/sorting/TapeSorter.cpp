...
TapeSorter::TapeSorter(std::unique_ptr<TapeI> input, std::unique_ptr<TapeI> output, SorterConfig config)
    :
    ...
    if (config.run_seq) {
        maxChunkElements_ =
            static_cast<size_t>(config.memoryLimitBytes * config.memoryUtilizationFactor / sizeof(int32_t));
    } else {
        maxChunkElements_ = static_cast<size_t>(
            config.memoryLimitBytes * config.memoryUtilizationFactor / (sizeof(int32_t) * memory_divide_coef)
        );
    }
}
    ...
    std::thread writer([&]() {
        try {
            while (true) {
                ...
                if (!sortedBlocks.pop(sortedBlock)) {
                    break;
                }
                if (errorOccurred) {
                    break;
...
MinHeap<std::pair<std::int32_t, size_t>> TapeSorter::fillTempHeap() {
    MinHeap<std::pair<std::int32_t, size_t>> heap(
        tempTapes_.size(),
        [](const std::pair<int32_t, size_t> &a, const std::pair<int32_t, size_t> &b) { return a.first < b.first; }
    );
